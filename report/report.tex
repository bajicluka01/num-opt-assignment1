...
\subsection*{a) $f(\textbf{x})=\frac{1}{4}\|\textbf{x}-\textbf{b}\|^4 \text{ for } \textbf{b}\in\mathbb{R}^n$}

\subsection*{b) $f(\textbf{x})=\sum_{i=1}^ng((\textbf{Ax})_i) \text{ with } g(z)=\frac{1}{2}z^2+z \text{ for } z\in\mathbb{R}, \textbf{A}\in\mathbb{R}^{n\times n}$, subscript $i$ denoting the $i$-th element}

\subsection*{c) $f(\textbf{x})=(\textbf{x}\oslash\textbf{b})^{\text{T}}\textbf{D}(\textbf{x}\oslash\textbf{b}) \text{ for } \textbf{b}\in\mathbb{R}^n, \mathbb{D}\in\mathbb{R}^{n\times n}$}
